...
static char *buses[] = {
    "CBUSI ", "CBUSII", "EISA  ", "FUTURE", "INTERN", "ISA   ", "MBI   ",
    "MBII  ", "MCA   ", "MPI   ", "MPSA  ", "NUBUS ", "PCI   ", "PCMCIA",
    "TC    ", "VL    ", "VME   ", "XPRESS", 0,
};
...
static void lapic_write(int r, int data) {
...
void lapic_init(int c) {
  ...
  lapic_write(LAPIC_ICRLO,
              LAPIC_ALLINC | APIC_LEVEL | LAPIC_DEASSERT | APIC_INIT);
...
static void lapic_startap(struct cpu *c, int v) {
  ...
  lapic_write(LAPIC_ICRLO,
              LAPIC_FIELD | APIC_LEVEL | LAPIC_DEASSERT | APIC_INIT);
  ...
  for (i = 0; i < 1; i++) {
    lapic_write(LAPIC_ICRHI, crhi);
    lapic_write(LAPIC_ICRLO,
                LAPIC_FIELD | APIC_EDGE | APIC_STARTUP | (v / PGSIZE));
    while (j++ < 100000) {
      ;
    }
  }
...
static struct MP *mp_scan(uint8_t *addr, int len) {
  uint8_t *e, *p, sum;
  int i;
