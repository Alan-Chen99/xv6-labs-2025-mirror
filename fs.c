...
struct inode *igetroot(void) { return iget(ROOTDEV, 1); }
...
uint bmap(struct inode *ip, uint bn, int alloc) {
  uint addr, *a;
  struct buf *bp;
...
//   return 0.
int dirlookup(struct inode *dp, char *name, int namelen, uint *poff,
              uint *pinum) {
  uint off;
  struct buf *bp;
  struct dirent *de;
  ...
  for (off = 0; off < dp->size; off += BSIZE) {
    ...
    for (de = (struct dirent *)bp->data;
         de < (struct dirent *)(bp->data + BSIZE); de++) {
      if (de->inum == 0)
        continue;
...
void dirwrite(struct inode *dp, char *name, int namelen, uint ino) {
  int off;
  struct dirent de;
...
struct inode *dircreat(struct inode *dp, char *name, int namelen, short type,
...
//   skipelem("") = skipelem("////") = 0
//
static char *skipelem(char *path, char **name, int *len) {
  while (*path == '/')
    path++;
  if (*path == 0)
    return 0;
...
struct inode *_namei(char *path, int parent, char **pname, int *pnamelen) {
  struct inode *dp;
  char *name;
  int namelen;
  uint off, dev, inum;
...
struct inode *namei(char *path) { return _namei(path, 0, 0, 0); }

struct inode *nameiparent(char *path, char **name, int *namelen) {
