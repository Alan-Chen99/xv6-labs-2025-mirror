...
void kvminit() {
  ...
  // PLIC
  mappages(kernel_pagetable, PLIC, 0x4000000, PLIC, PTE_R | PTE_W);
...
char *mapkstack(uint64 kstack) {
  char *k = kalloc();
  if (k == 0) {
    return 0;
  }
  if (mappages(kernel_pagetable, kstack, PGSIZE, (uint64)k, PTE_R | PTE_W) ==
      0) {
    kvminithart();
    return (char *)kstack;
  }
