...
struct segdesc bootgdt[NSEGS] = {
    ...
    [1] = SEGDESC(0, 0xfffff,
                  SEG_R | SEG_CODE | SEG_S | SEG_DPL(0) | SEG_P | SEG_D |
                      SEG_G), // 32-bit kernel code
    [2] = SEGDESC(0, 0,
                  SEG_R | SEG_CODE | SEG_S | SEG_DPL(0) | SEG_P | SEG_L |
                      SEG_G), // 64-bit kernel code
    [3] = SEGDESC(0, 0xfffff,
                  SEG_W | SEG_S | SEG_DPL(0) | SEG_P | SEG_D |
                      SEG_G), // kernel data
    // The order of the user data and user code segments is
    // important for syscall instructions.  See initseg.
    [6] = SEGDESC(0, 0xfffff,
                  SEG_W | SEG_S | SEG_DPL(3) | SEG_P | SEG_D |
                      SEG_G), // 64-bit user data
    [7] = SEGDESC(0, 0,
                  SEG_R | SEG_CODE | SEG_S | SEG_DPL(3) | SEG_P | SEG_L |
                      SEG_G), // 64-bit user code
};
...
void seginit(void) {
  ...
  uint64 star =
      ((((uint64)SEG_UCODE | 0x3) - 16) << 48) | ((uint64)(SEG_KCODE) << 32);
...
static pte_t *walkpgdir(pde_t *pml4, const void *va, int alloc) {
  pde_t *pgtab = pml4;
  pde_t *pte;
  int level;
...
// be page-aligned.
static int mappages(pde_t *pgdir, void *va, uint64 size, uint64 pa, int perm) {
  char *a, *last;
  pte_t *pte;
...
int deallocuvm(pde_t *pml4, uint64 oldsz, uint64 newsz) {
  pte_t *pte;
  uint64 a, pa;
...
void freelevel(pde_t *pgtab, int level) {
  int i;
  pde_t *pd;
...
void freevm(pde_t *pml4, uint64 sz) {
  if (pml4 == 0)
