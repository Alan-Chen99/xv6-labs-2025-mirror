...
void kvminit() {
  ...
  mappages(kernel_pagetable, KERNBASE, (uint64)etext - KERNBASE, KERNBASE,
           PTE_R | PTE_X);
  ...
  mappages(kernel_pagetable, TRAMPOLINE, PGSIZE, (uint64)trampstart,
           PTE_R | PTE_X);
...
static pte_t *walk(pagetable_t pagetable, const void *va, int alloc) {
  if ((uint64)va >= MAXVA)
    panic("walk");
...
// physical memory.
void unmappages(pagetable_t pagetable, uint64 va, uint64 size, int do_free) {
  char *a, *last;
  pte_t *pte;
  uint64 pa;
...
int uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz) {
  if (newsz >= oldsz)
    return oldsz;
