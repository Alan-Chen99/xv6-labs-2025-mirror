...
static int mappages(pde_t *pgdir, void *la, uint size, uint pa, int perm,
  ...
  for (i = 0; i < size; i += PGSIZE) {
    ...
    *pte = (pa + i) | perm | PTE_P;
    if (p)
      cprintf("mappages 0x%x 0x%x pp %d\n", la + i, *pte, PPN(*pte));
  }
  return 1;
}
...
int allocuvm(pde_t *pgdir, char *addr, uint sz) {
  uint i, n;
  char *mem;
...
void inituvm(pde_t *pgdir, char *addr, char *init, uint sz) {
  uint i, pa, n, off;
  pte_t *pte;
...
void pminit(void) {
  ...
  cprintf("kerntext@0x%x(sz=0x%x), kerndata@0x%x(sz=0x%x), kernend 0x%x freesz "
          "= 0x%x\n",
...
void loadkvm(void) { lcr3(PADDR(kpgdir)); }
