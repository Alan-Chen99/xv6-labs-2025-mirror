...
int main(void) {
  ...
  enterothers(); // start other processors (must come before kinit; must use
                 // boot_alloc)
...
static void enterothers(void) {
  ...
  for (c = cpus; c < cpus + ncpu; c++) {
    ...
    // above 4Mbyte (the machine may have much more physical memory than
    // 4Mbyte), which aren't mapped by bootpgdir, so we must allocate a stack
    // using boot_alloc(); This introduces the constraint that xv6 cannot use
    // kalloc until after these last boot_alloc invocations.
    stack = boot_alloc();
...
__attribute__((__aligned__(PGSIZE))) pde_t bootpgdir[NPDENTRIES] = {
