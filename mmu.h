...
#define SEGDESC(base, limit, bits)                                             \
  (struct segdesc) {                                                           \
    (limit) & 0xffff, (base) & 0xffff, ((base) >> 16) & 0xff, (bits) & 0xff,   \
        (((bits) >> 4) & 0xf0) | ((limit >> 16) & 0xf), ((base) >> 24) & 0xff, \
  }

// SEGDESCHI constructs an extension segment descriptor
// literal that records the high bits of base.
#define SEGDESCHI(base)                                                        \
  (struct segdesc) { (((base) >> 32) & 0xffff), (((base) >> 48) & 0xffff), }
...
#define PXMASK 0x1FF
#define PXSHIFT(n)                                                             \
  (PGSHIFT + (9 * (n))) // shift for index into level n page table
